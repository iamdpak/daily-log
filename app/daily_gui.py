#!/usr/bin/env python3
"""
Daily Check-in (class-based)
- Tab 1: Check-in (textarea + password) → encrypt → upload to Google Drive AppDataFolder
- Tab 2: Viewer (date picker) → list matching files → download → decrypt → show
- End-to-end encryption: Argon2id (KDF) + NaCl SecretBox
- One-stamp-per-day logic so the noon popup only occurs once (works nicely with systemd timers)
"""
...
class AppConfig:
    ...
    token_path: Path = (Path.home() / ".daily_checkin_drive" / "token.json")
    salt_path: Path = (Path.home() / ".daily_checkin_drive" / "kdf_salt.bin")
    stamp_path: Path = (Path.home() / ".daily_checkin_drive" / "last_submit_date.txt")
    scopes: tuple = ("https://www.googleapis.com/auth/drive.appdata",)
    timezone: str = "Australia/Sydney"   # bucket + display TZ
...
class CryptoManager:
    """
    Handles password-based key derivation and symmetric encryption/decryption.
    Format stored: b'v1' + salt(16) + secretbox(nonce+ct)
    """
    def __init__(self, cfg: AppConfig):
        self.cfg = cfg
        self.cfg.app_dir.mkdir(exist_ok=True, parents=True)
...
class DriveClient:
    """Thin wrapper around Google Drive AppDataFolder API."""
    def __init__(self, cfg: AppConfig):
        self.cfg = cfg
        self._service = None
    ...
    def download(self, file_id: str) -> bytes:
        """Robust download using chunked MediaIoBaseDownload."""
...
class DailyController:
    ...
    def load_entries_for_date(self, passphrase: str, target: date):
        if not passphrase:
...
class DailyApp(tk.Tk):
    ...
    def _build_checkin_tab(self):
        ...
        ttk.Label(row, text="Encryption password:").pack(side=tk.LEFT)
    ...
    def _on_load_day(self):
        pw = self.passphrase.get().strip()
        if not pw:
            messagebox.showinfo("Password needed", "Enter your encryption password on the Check-in tab.")
            return
